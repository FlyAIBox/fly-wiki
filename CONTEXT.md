...
> **一期范围标注**：执行者为个人开发者 + AI 辅助。标注 `（二期）` 的术语保留领域定义与数据结构边界，但不进入一期交付；它们主要服务于对外分享、写作成稿、本地守护进程与多渠道产品化能力。
...
**Belief**:
用户明确表达或确认过的个人判断及其适用边界。一期最简形态：Belief 由 Workspace Knowledge Status 的“接受”状态直接承担——用户对某个 Claim 标记接受，该 Claim 即成为其 Belief。系统只能提出修改建议，不能自动改变 Belief。
_Avoid_: 用户画像、系统结论、推断认知

**Profile Document**（二期）:
用户可编辑、可版本化的身份、兴趣、记忆或认知记录。一期不设独立 Profile Document 体系，其职责由 [[Belief]] 的最简形态承担。
...
**Knowledge Inbox**:
集中呈现 Change Proposal、健康问题和异常任务的待处理视图。它是处理入口，不是新的知识存储。（访客投稿与记忆候选属于二期。）
_Avoid_: 消息中心、通知列表
...
**Knowledge Release**（二期）:
Knowledge Base 用于分享和查询的不可变发布快照。新的发布原子替换默认指针，旧发布可保留、回退或被强制下架。一期知识回滚由 [[Knowledge Change Set]] 承担，不依赖 Release 快照。
...
**Channel Binding**:
平台账号、Channel Principal、会话范围和授权能力之间的可撤销关系。一期用于飞书与开发者自用微信桥接。
_Avoid_: 登录、好友关系

**Edge Device**（二期）:
运行在用户设备或私有服务器上的受限本地连接器，负责本地文件同步、个人微信接入和本地密钥保管，不运行通用 Agent。一期不做本地守护进程，微信仅为开发者自用桥接。
...
针对引用、来源新鲜度、冲突、结构或编译状态发现的可解释问题（一期六维，不含发布健康）。Finding 必须指向具体对象和建议动作。
