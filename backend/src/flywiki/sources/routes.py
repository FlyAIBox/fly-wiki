...
async def retry_capture(
    ...
    # updated_at is generated by the database on UPDATE and is therefore
    # expired after commit. Refresh it while still in an async IO boundary so
    # Pydantic does not trigger an implicit lazy load during serialization.
