...
| Domain | Command | Why |
|--------|---------|-----|
| `mp.weixin.qq.com` | `fetch.py <url>` | Fast mode usually produces cleaner article Markdown; automatically falls back to stealth when needed |
...
# Force stealth only when the fast result is incomplete
...
# JSON output with metadata (url, mode, selector, content_length, metadata, content)
...
For stealth mode, install the browser once with `python3 -m patchright install chromium`.

## FlyWiki Adapter Contract

FlyWiki invokes this skill only for `mp.weixin.qq.com/s/...` URLs through its
`WeChatPublicAccountFetcher`. Keep `scripts/fetch.py <url> <max_chars> --json`
backward compatible, including its fast-to-stealth fallback. JSON output must
contain `content` and may include `metadata.title`,
`metadata.author`, and `metadata.published_at`. Failures must exit non-zero or
return an `error` field so FlyWiki can continue to the generic capture fallback.
